...
//     // For internal JWT, we need to fetch the full user from the database
...
const authGatekeeper = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      return next(new AppError('User associated with this token not found.', 401));
    }
    ...
    // This is the expected path for the Mobile Client using Auth0.
    console.log('[Gatekeeper] Internal JWT check failed. Assuming Auth0 token and proceeding to verify...');
  }
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      auth0Middleware(req, res, (err) => {
        if (err) {
          // If Auth0 middleware finds an error, we reject.
          return reject(new AppError('Auth0 authentication failed.', 401));
        }
        // If it succeeds, it populates req.auth and we resolve.
        resolve();
      });
    ...
    // If the promise was rejected, the Auth0 validation failed.
    // The `catch` block below will handle this.
    ...
    // This will now correctly catch the rejection from the Promise above.
    console.error('[Gatekeeper] FATAL: Authentication failed.', error.message);
