...
const getUserId = async (req) => {
  try {
    ...
    if (req.auth?.payload?.sub) {
      ...
      if (!user) throw new AppError('User not found for the given Auth0 ID.', 404);
      return user.id;
    }
    ...
    console.error('[DietController] Error in getUserId:', error);
    throw error;
  }
};
...
export const logDietEntry = catchAsync(async (req, res) => {
  ...
  const { mealName, mealType, calories, protein, carbs, fats, photoUrl, notes } = req.body;
  ...
  res.status(201).json({ 
    success: true, 
    message: 'Diet entry logged successfully.', 
    data: newEntry 
  });
});
